...
const ParlayDetailScreen = ({ navigation, route }) => {
  ...
  const overallStrength =
    betData.length > 0 && betData.response != null
      ? betData[0].response.over_under_analysis
      : "C";
  ...
  const handleEvaluateBet = (response) => {
    ...
    console.log(...betData);
    console.log("Setting Bet Data...\n\n");
    const newBetData = [newBet, ...betData];
              ...
              <View style={styles.betText}>
                <View style={styles.betTextContainer}>
                  <Text style={styles.betTitle}>
                    {bet.title || "NIKOLA JOKIC"}:{" "}
                  </Text>
                  {bet.response != null ? (
                    <Text
                      style={getStyleForGrade(bet.response.over_under_analysis)}
                    >{`(${bet.response.over_under_analysis})`}</Text>
                  ) : (
                    <Text style={getStyleForGrade("C")}>C</Text>
                  )}
                </View>
